...
- `POST /sessions` — `{ adminName, pointSystem, sliderMax }` → `201 { sessionId, adminToken, adminParticipantId, adminName, pointSystem }`. Name (`/^[A-Za-z0-9]{1,20}$/`) and scale-bound validation both happen inside `sessionStore.createSession` itself, not in the route — the route just calls it and lets a thrown `AppError` propagate. This isn't just tidiness: `POST /sessions` is not the only place a participant name is submitted — the WS `join` event (below) is the *only* entry point for every non-admin participant, so name validation has to live somewhere both transports go through, or the WS path silently accepts anything.
